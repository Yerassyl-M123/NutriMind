...
func SignIn(c *gin.Context) {
	...
	err := config.DB.QueryRow("SELECT id, birth_date, email, password, role FROM users WHERE email = $1", email).Scan(&storedUser.ID, &storedUser.BirthDate, &storedUser.Email, &storedUser.Password, &storedUser.Role)
	if err != nil {
...
func SignUp(c *gin.Context) {
	...
	err = config.DB.QueryRow("INSERT INTO users (birth_date, email, password) VALUES ($1, $2, $3) RETURNING id", BirthDate, userEmail, hashedPassword).Scan(&newUser.ID)
	if err != nil {
		log.Fatal(err)
	}
